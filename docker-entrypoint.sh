...
cat > /app/find-journal.js << 'EOFJOURNAL'
const fs = require('fs');
...
cat > /app/migration-patch.js << 'EOFMIGRATION'
const fs = require('fs');
const path = require('path');
...
if [ ! -f "src/pages/_document.tsx" ] && [ ! -f "src/pages/_document.js" ]; then
  ...
  mkdir -p src/pages
  cat > src/pages/_document.tsx << 'EOFDOC'
...
cat > /app/inject-inline-css.js << 'EOFINLINE'
const fs = require('fs');
const path = require('path');
...
if [ -d ".next" ]; then
  ...
  if [ -z "$(find .next -name "*.css" 2>/dev/null)" ]; then
    ...
    cat > .next/static/css/fallback.css << 'EOFFALLBACK'
...
EOFFALLBACK
    ...
    cat > /app/inject-css.js << 'EOFINJECTOR'
    const fs = require('fs');
    const path = require('path');
